...
def loader_WADI_OCC(root, batch_size, window_size, stride_size, train_split, label=False):
    ...
    if label:
        train_loader = DataLoader(WADI_dataset(train_df, train_label, window_size, stride_size), batch_size=batch_size,
                                  shuffle=False)
    else:
        train_loader = DataLoader(WADI_dataset(train_df, train_label, window_size, stride_size), batch_size=batch_size,
                                  shuffle=True)
    val_loader = DataLoader(WADI_dataset(val_df, val_label, window_size, stride_size), batch_size=batch_size,
                            shuffle=False)
    test_loader = DataLoader(WADI_dataset(test_df, test_label, window_size, stride_size), batch_size=batch_size,
                             shuffle=False)

    return train_loader, val_loader, test_loader, n_sensor


def loader_WADI(batch_size, window_size, stride_size, train_split, label=False, k=10, alpha=0.5, seed=15):
    data = pd.read_csv("Data/input/WADI_attackdata.csv", sep=",")  # , nrows=1000)
    labels = []
    ...
    if label:
        train_loader = DataLoader(
            WADI_dataset(train_df, train_label, window_size, stride_size, train_split=train_split, train='train', k=k,
                         alpha=alpha),
            batch_size=batch_size, shuffle=False
        )
        # train_dataset = WADI_dataset(train_df, train_label, window_size, stride_size, train_split=train_split, train='train', k=k, alpha=alpha)
    else:
        train_loader = DataLoader(
            WADI_dataset(train_df, train_label, window_size, stride_size, train_split=train_split, train='train', k=k,
    ...
    #     WADI_dataset(val_df, val_label, window_size, stride_size, train_split=train_split, train='val', k=k, alpha=alpha),
    #     batch_size=batch_size, shuffle=False, num_workers=4, pin_memory=True, persistent_workers=True)

    test_loader = DataLoader(
        WADI_dataset(test_df, test_label, window_size, stride_size, train_split=train_split, train='test', k=k,
                     alpha=alpha, seed=seed),
        batch_size=batch_size, shuffle=False, num_workers=4, pin_memory=False, persistent_workers=True)
    # test_dataset = WADI_dataset(test_df, test_label, window_size, stride_size, train_split=train_split, train='test', k=k, alpha=alpha)
...
class WADI_dataset(Dataset):
    def __init__(self, df, label, window_size=60, stride_size=10, k=10, alpha=0.5, train_split=0.8,
        ...
        filename = "save_near_index/WADI_near{}_train{}_windowsize{}_trainsplit{}_seed{}.pkl".format(k, self.train,
                                                                                                     self.window_size,
                                                                                                     self.train_split,
                                                                                                     self.seed)
