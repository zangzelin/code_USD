...
class DROCCTrainer:
    ...
    def train(self, args, seed, train_loader, test_loader, lr_scheduler, total_epochs, save_path, name,
                only_ce_epochs=20, ascent_step_size=0.001, ascent_num_steps=50):
        ...
        for epoch in range(40): 
            ...
            gt, pre = self.test(test_loader)
            logger.print_result(gt, pre, self.model, seed, args)
    ...
    def test(self, test_loader):
        ...
        # auc_score = roc_auc_score(np.asarray(test_loader.dataset.label,dtype=int),scores)
        # print('roc_test', auc_score)
        return np.asarray(test_loader.dataset.label,dtype=int), scores
